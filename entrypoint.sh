...
# Import publish key
echo $PUBLISH_KEY_VALUE >> $PUBLISH_KEY_NAME.pam
ipfs key import --format=pem-pkcs8-cleartext $PUBLISH_KEY_NAME $PUBLISH_KEY_NAME.pam

# Unpin previous hash from remote  
IPNS=$(ipfs key list -l | head -2 | tail -1 | awk '{printf $1}')
OLD_CID=$(ipfs name resolve $IPNS | sed 's/\/ipfs\///')
ipfs pin remote rm --service=$REMOTE_SERVICE_NAME $OLD_CID
